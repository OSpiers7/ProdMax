...
// Pause timer session
router.put('/pause/:id', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.params.id) {
      return res.status(400).json({
        success: false,
        error: 'Timer session ID is required'
      });
    }
...
// Stop timer session
router.put('/stop/:id', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.params.id) {
      return res.status(400).json({
        success: false,
...
// Start break session
router.post('/break/:id', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
